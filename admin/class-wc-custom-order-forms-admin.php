...
class Wc_Custom_Order_Forms_Admin
{
	...
	public function register_custom_order_plug_shortcodes()
	{
		...
		add_shortcode('shipping_address', array($this, 'wc_order_form_shipping_address'));
	}

	/**
	 * Previous suite number generation
	 */
	public static function generateSuiteNumber($id, $countrycode)
	{
		// $countryCodeRef = ['EG'=> '020', 'AE'=> '971', 'LK' => '000'];
		$countryCodeRef = [
			'AF' => '093', 'AL' => '0355', 'DZ' => '0213', 'AS' => '01-684', 'AD' => '0376', 'AO' => '0244',
		...
		$suitenumber 	= (get_user_meta($id, 'suite_number', true)) ? get_user_meta($id, 'suite_number', true) : ($countrycode ? $countryCodeRef[$countrycode] . '-' . $suite : $suite);
		return $suitenumber;
	}

	public static function generateNewSuiteNumber($id)
	{
		$length = 4;
		if (strlen($id) <= '3') {
			$zerolength = 3 - strlen($id);
			$suite = substr('1' . str_repeat(0, $zerolength) . $id, -$length);
		} else {
			$firstletter = substr($id, 0, 1);
			$ending = substr($id, 1);
			$suite = ($firstletter + 1) . $ending;
		}
		$prefix = substr(date("Y"), -2);
		$suitenumber 	= (get_user_meta($id, 'suite_number', true)) ? get_user_meta($id, 'suite_number', true) : $prefix . '-' . $suite;
		return $suitenumber;
	}
	...
	public function wc_order_form_buy()
	{
		...
		if (is_user_logged_in()) {
			...
			$suitenumber = $this->generateNewSuiteNumber($id);
		}
		...
		$form_buy .= '<h2 class="labeltitle">Customer details</h2>';

		$form_buy .= '<div class="form-row block-element">';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="name">First Name:</label>';
		$form_buy .= '<input class="form-control" type="text" name="name" value="' . $name . '"/>';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="last">Last Name:</label>';
		$form_buy .= '<input class="form-control" type="text" name="last" value="' . $last . '"/>';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="contact_number">Contact Number: </label>';
		$form_buy .= '<input class="form-control" type="text" name="contact_number" value="' . $phone . '"/>';
		$form_buy .= '</div>';
		$form_buy .= '</div>';
		...
		$form_buy .= '<h2 class="labeltitle">Shipping details</h2>';
		// $form_buy .= '<h2 class="labeltitle" style="display:inline-block;">Shipping details</h2>';	
		$form_buy .= '<div class="form-row block-element">';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="shipping_address">Street address: </label>';
		$form_buy .= '<input class="form-control" type="text" name="shipping_address_01" value="' . $address_1 . '" placeholder="House number and street name" required/>';
		// $form_buy .= '<input class="form-control" type="text" name="shipping_address_02" value="'.$address_2.'" placeholder="Apartment, suite, unit, etc. (optional)"/>';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="shipping_address">Town / City : </label>';
		$form_buy .= '<input class="form-control" type="text" name="shipping_address_city" value="' . $city . '" placeholder="Town / city" />';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-6">';
		$form_buy .= '<label label-for="shipping_address">Postcode / ZIP : </label>';
		$form_buy .= '<input class="form-control" type="text" name="shipping_address_postcode" value="' . $postcode . '" placeholder="Postcode / ZIP"/>';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col-md-6">';
		...
		foreach ($countries as $key => $country) {
			$form_buy .= '<option ' . selected($userCountry, $country, false) . ' value=' . $key . '>' . $country . '</option>';
		}
		$form_buy .= '</select>';
		$form_buy .= '</div>';
		$form_buy .= '</div>';
		...
		$form_buy .= '<h2 class="labeltitle">Product details</h2>';

		$form_buy .= '<div class="input-div" style="display: inline-block;width: 100%;">';
		...
		$form_buy .= '<input type="file" class="form-control-file" name="item_photo[]">';
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-12">';
		...
		$form_buy .= '</div>';
		$form_buy .= '<div class="col col-md-12">';
		$form_buy .= '<label label-for="coupon_code">Coupon Code: </label>';
		$form_buy .= '<input class="form-control" type="text" name="coupon_code"/>';
		$form_buy .= '</div>';
		...
		if (is_user_logged_in()) {
			...
			return "You need to logged in to website to view this page";
		}
	}
	...
	public function wc_order_form_ship()
	{
		...
		$form_ship .= '<h2 class="labeltitle">Customer details</h2>';
		$form_ship .= '<div class="form-row block-element">';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="name">First Name: </label>';
		$form_ship .= '<input class="form-control" type="text" name="name" value="' . $name . '" />';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="last">Last Name: </label>';
		$form_ship .= '<input class="form-control" type="text" name="last" value="' . $last . '" />';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="contact_number">Contact Number: </label>';
		$form_ship .= '<input class="form-control" type="text" name="contact_number" value="' . $phone . '" />';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		...
		$form_ship .= '<h2 class="labeltitle">Shipping details</h2>';
		$form_ship .= '<div class="form-row block-element">';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="shipping_address">Street address: </label>';
		$form_ship .= '<input class="form-control" type="text" name="shipping_address_01" value="' . $address_1 . '" placeholder="House number and street name" required/>';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="shipping_address">Town / City : </label>';
		$form_ship .= '<input class="form-control" type="text" name="shipping_address_city" value="' . $city . '" placeholder="Town / city"/>';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="shipping_address">Postcode / ZIP : </label>';
		$form_ship .= '<input class="form-control" type="text" name="shipping_address_postcode" value="' . $postcode . '" placeholder="Postcode / ZIP"/>';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		...
		foreach ($countries as $key => $country) {
			$form_ship .= '<option ' . selected($userCountry, $country, false) . ' value=' . $key . '>' . $country . '</option>';
		}
		$form_ship .= '</select>';
		$form_ship .= '</div>';
		$form_ship .= '</div>';
		...
		$form_ship .= '<h2 class="labeltitle">Product details</h2>';

		$form_ship .= '<div class="input-div" style="display: inline-block;width: 100%;">';
		...
		$form_ship .= '<input type="file" class="form-control-file" name="item_photo[]">';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-12">';
		...
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-12">';
		$form_ship .= '<label label-for="coupon_code">Coupon Code: </label>';
		$form_ship .= '<input class="form-control" type="text" name="coupon_code"/>';
		$form_ship .= '</div>';
		$form_ship .= '</div>';
	...
	public function wc_order_form_ondemad()
	{
		...
		// $form_ship .= '<h2 style="display:inline-block;">Shipping details</h2>';	
		$form_ship .= '<h2 class="labeltitle">Shipping details</h2>';
		$form_ship .= '<div class="form-row block-element">';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="shipping_address">Street address: </label>';
		$form_ship .= '<input class="form-control" type="text" name="shipping_address_01" value="' . $address_1 . '" placeholder="House number and street name" required/>';
		// $form_ship .= '<input class="form-control" type="text" name="shipping_address_02" value="'.$address_2.'" placeholder="Apartment, suite, unit, etc. (optional)"/>';
		$form_ship .= '</div>';
		$form_ship .= '<div class="col col-md-6">';
		$form_ship .= '<label label-for="shipping_address">Town / City : </label>';
		$form_ship .= '<input class="form-control" type="text" name="shipping_address_city" value="' . $city . '" placeholder="Town / city" />';
	...
	public function wc_order_form_shipping_address()
	{
		...
		$suitenumber	= Wc_Custom_Order_Forms_Admin::generateNewSuiteNumber($id);
		...
		// return '<address class="shipping_address"><strong>Your TAX Free US Address</strong><span id="clk_2_copy_add"><br/>'.$name.',<br/>2230 Middlegreen Ct,<br/>Suite '.$suitenumber.',<br/>Lancaster PA, 17601,<br/>USA</span></address>';
		return '<address class="shipping_address"><strong>Your TAX Free US Address</strong><span id="clk_2_copy_add"><br/><br/>' . $name . ',<br/>421 PRESCOT ST,<br/>Suite ' . $suitenumber . ',<br/>Lancaster PA, 17601,<br/>USA</span></address>';
	}
	...
	function custom_order_settings_init()
	{
		...
		add_settings_field(
			'wc_order_custom_form_setting_field_3',
			'On demand page',
			array($this, 'wc_order_custom_form_setting_markup_3'),
			'wc-custom-order-page',
			'wc_order_setting_section'
		);
		...
		register_setting('wc-custom-order-page', 'wc_order_custom_form_setting_field_3');
	}
...
?>
	...
	function wc_order_custom_form_setting_markup_3()
	{
		$ondemand = get_option('wc_order_custom_form_setting_field_3');
	?>
		<select id="wc_order_custom_form_setting_field_3" name="wc_order_custom_form_setting_field_3">
			<option value=""><?php echo esc_attr(__('Select page')); ?></option>
			<?php
			$pages = get_pages();
			foreach ($pages as $page) {
				$option = '<option value="' . $page->ID . '" ' . selected($ondemand, $page->ID) . '>';
				$option .= $page->post_title;
				$option .= '</option>';
				echo $option;
			}
...
<?php
	...
	function custom_order_my_account_link($menu_links)
	{
		...
		$new = array('buy4me' => 'Buy for me ', 'packAndship' => 'PackNShip', 'ondemand' => 'OnDemand');
	...
	function custom_order_hook_endpoint($url, $endpoint, $value, $permalink)
	{
		...
		if ($endpoint === 'ondemand') {
			$ondemand = get_option('wc_order_custom_form_setting_field_3');
			// ok, here is the place for your custom URL, it could be external
			$url = get_permalink($ondemand);
			//$url = site_url();
		}
		return $url;
	}
